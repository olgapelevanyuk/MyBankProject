...
class Page_Login extends React.PureComponent {
  ...
  validateLogin = (e, withValidate = false) => {
    ...
    if (
      withValidate &&
      value &&
      !this.props.users.list.some((user) => value === user.login)
    ) {
  ...
  validatePassword = (e, withValidate = false) => {
    ...
    if (
      ...
      !users.list.some(
        (user) =>
          this.state.login.value === user.login && value === user.password
      )
    ) {
    ...
    this.setState({ ...this.state, password: newState });
  };
  ...
  render() {
    ...
    const formValid =
      ...
      this.props.users.list.some(
        (user) =>
          user.login === this.state.login.value &&
    ...
    return (
      (this.props.users.loading && <div>Идет загрузка...</div>) || (
        <div className="registrWrapper">
          ...
          <div>
            <button
              onClick={formValid ? this.login : null}
